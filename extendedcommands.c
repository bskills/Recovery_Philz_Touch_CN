...
static void show_partition_sdcard_menu() {
    ...
    const char* ext_headers[] = { "Ext 大小", "", NULL };
    const char* swap_headers[] = { "Swap 大小", "", NULL };
...
void wipe_data_menu() {
    const char* headers[] = { "选择擦除选项", NULL };
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 1: {
                if (confirm_selection("擦除缓存分区 ?", "是的 - 擦除缓存")) {
                    ui_print("\n-- 正在擦除缓存...\n");
            ...
            case 2: {
                ...
                if (confirm_selection("擦除 dalvik 缓存 ?", "是的 - 擦除 dalvik 缓存")) {
                    ui_print("\n-- 正在擦除 dalvik 缓存...\n");
                    __system("rm -r /data/dalvik-cache");
                    __system("rm -r /cache/dalvik-cache");
                    __system("rm -r /sd-ext/dalvik-cache");
                    ui_print("Dalvik 缓存已擦除.\n");
                }
                break;
            }
            case 3: {
                ...
                if (confirm_with_headers(headers, "是的 - 擦除用户和系统数据")) {
                    wipe_data(false);
                    ui_print("-- 擦除系统数据中...\n");
                    erase_volume("/system");
            ...
            case 4: {
                ...
                if (confirm_with_headers(headers, "是的 - 擦除所有媒体文件")) {
                    ui_print("\n-- 擦除媒体文件中...\n");
...
void show_multi_flash_menu() {
    const char* headers_dir[] = { "选择一个ZIP压缩文件", NULL };
    const char* headers[] = { "选择将要安装文件...", NULL };
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        if (no_files_found) {
            ui_print("子文件夹下至少需要创建一个ZIP压缩文件 %s\n", tmp);
            ui_print("在其他存储中查找...\n");
        }
    } else {
    ...
    if (zip_folder == NULL) {
        if (!(S_ISDIR(st.st_mode)) && !(S_ISDIR(s.st_mode)))
            ui_print("至少创建一个子文件夹放置ZIP压缩文件 %s\n", MULTI_ZIP_FOLDER);
        return;
    }
    ...
    if (numFiles == 0) {
        ...
        if (chosen_item == 1) {
            ...
            if (confirm_selection("安装选择文件?", confirm)) {
                for(i = 2; i < numFiles + 2; i++) {
...
static void choose_ors_volume() {
    ...
    const char* headers[] = { "保存 ors 备份到:", NULL };
    ...
    for (i=0; i<line_len; i++) {
        if (value1[i] == 'S' || value1[i] == 's') {
            ...
            if (nandroid_add_preload.value) {
                backup_preload = 1;
                ui_print("nandroid设置允许预置.\n");
                ui_print("这将与 /system 一起处理\n");
            }
        ...
        while (fgets(script_line, SCRIPT_COMMAND_SIZE, fp) != NULL && ret_val == 0) {
            ...
            if (strcmp(command, "install") == 0) {
                ...
                if (strcmp(value, "cache") == 0 || strcmp(value, "/cache") == 0) {
                    ...
                    ui_print("-- Dalvik 缓存擦除完成!\n");
                } else if (strcmp(value, "data") == 0 || strcmp(value, "/data") == 0 || strcmp(value, "factory") == 0 || strcmp(value, "factoryreset") == 0) {
                    ui_print("-- 正在擦除 DATA 分区...\n");
                    wipe_data(0);
                    ui_print("-- Data 分区擦除完成!\n");
                } else {
                ...
                tok = strtok(NULL, " ");
                strcpy(value2, tok);
                ui_print("设置命令不允许在 CWMR: '%s' to '%s'\n", value1, value2);
            } else if (strcmp(command, "mkdir") == 0) {
                // Make directory (recursive)
                ui_print("递归创建文件夹不允许在 CWMR: '%s'\n", value);
...
static void choose_default_ors_menu(const char* volume_path) {
    ...
    if (confirm_selection("确定运行脚本?", confirm)) {
...
static void choose_custom_ors_menu(const char* volume_path) {
    ...
    const char* headers[] = {"选择 .ors 脚本运行", NULL};
...
static void show_custom_ors_menu() {
    ...
    const char* headers[] = { "搜索 .ors 脚本运行", "", NULL };
...
static void regenerate_md5_sum_menu() {
    if (!confirm_selection("不推荐!!", "是的 - 创建新的 md5 数值"))
        return;
    ...
    char list_prefix[] = "从选择 ";
    char buf[80];
    const char* headers[] = {"重新生成 md5 数值", "选择一个备份文件生成", NULL};
    char* list[MAX_NUM_MANAGED_VOLUMES + 1];
    memset(list, 0, sizeof(list));
    sprintf(buf, "%s%s", list_prefix, primary_path);
    list[0] = strdup(buf);
    ...
    if (confirm_selection("重新生成 md5 数值 ?", tmp)) {
        if (0 == gen_nandroid_md5sum(backup_source))
            ui_print("重新生成 md5 数值完成.\n");
    }
...
void misc_nandroid_menu() {
    ...
    int fmt;
    for (;;) {
        if (enable_md5sum.value) ui_format_gui_menu(item_md5, "MD5 校验数值", "(x)");
        else ui_format_gui_menu(item_md5, "MD5 校验数值", "( )");
        ...
        if (twrp_backup_mode.value) ui_format_gui_menu(item_twrp_mode, "使用 TWRP 模式", "(x)");
        else ui_format_gui_menu(item_twrp_mode, "使用 TWRP 模式", "( )");

        if (show_nandroid_size_progress.value)
            ui_format_gui_menu(item_size_progress, "显示 Nandroid 大小进程", "(x)");
        else ui_format_gui_menu(item_size_progress, "显示 Nandroid 大小进程", "( )");
        list[3] = item_size_progress;

        if (use_nandroid_simple_logging.value)
            ui_format_gui_menu(item_use_nandroid_simple_logging, "使用简单的日志记录（快）", "(x)");
        else ui_format_gui_menu(item_use_nandroid_simple_logging, "使用简单的日志记录（快）", "( )");
        list[4] = item_use_nandroid_simple_logging;

        hidenandprogress = file_found(hidenandprogress_file);
        if (hidenandprogress) {
            ui_format_gui_menu(item_nand_progress, "隐藏 Nandroid 进程", "(x)");
            list[3] = NULL;
            list[4] = NULL;
        } else ui_format_gui_menu(item_nand_progress, "隐藏 Nandroid 进程", "( )");

        if (nand_prompt_on_low_space.value)
            ui_format_gui_menu(item_prompt_low_space, "当可用空间低时提示", "(x)");
        else ui_format_gui_menu(item_prompt_low_space, "当可用空间低时提示", "( )");

        char ors_volume[PATH_MAX];
        get_ors_backup_volume(ors_volume);
        ui_format_gui_menu(item_ors_path,  "ORS 备份包", ors_volume);
...
int show_custom_zip_menu() {
    const char* headers[] = { "选择一个ZIP文件应用", NULL };
...
*/
    ...
    if (chosen_item !=  GO_BACK) {
        ...
        sprintf(tmp, "是的 - 安装 %s", list[chosen_item]);
        ...
        confirm = confirm_selection("安装选择的文件吗?", tmp);
...
void show_twrp_restore_menu(const char* backup_volume) {
    ...
    const char* headers[] = { "选择一个备份文件恢复", NULL };
    ...
    char backup_source[PATH_MAX];
    sprintf(backup_source, "%s", DirName(file));
    ui_print("%s 将被恢复到选择的分区!\n", backup_source);
    sprintf(confirm, "是的 - 恢复 %s", BaseName(backup_source));
    if (confirm_selection("从这个备份文件中恢复吗 ?", confirm))
...
static void custom_restore_handler(const char* backup_volume, const char* backup_folder) {
    ...
    char backup_source[PATH_MAX];
    char* file = NULL;
    char* confirm_install = "从这个备份文件中恢复吗?";
    const char* headers[] = { "选择一个备份文件恢复", NULL };
    ...
    if (backup_efs == RESTORE_EFS_IMG) {
        ...
        ui_print("%s 将被刷写到 /efs!\n", backup_source);
        sprintf(tmp, "是的 - 恢复 %s", backup_source);
        ...
        if (file_found(tmp)) {
            ui_print("efs.img 文件检测到在 %s!\n", file);
            ui_print("选择 efs.img 同时恢复,\n");
            ui_print("或删除以恢复 nandroid source.\n");
        } else {
            // restore efs from nandroid tar format
            ui_print("%s 将被恢复到 /efs!\n", file);
            sprintf(tmp, "是的 - 恢复 %s", BaseName(file));
            if (confirm_selection(confirm_install, tmp))
        ...
        sprintf(backup_source, "%s", DirName(file));
        ui_print("%s 将被恢复到选择的分区!\n", backup_source);
        sprintf(tmp, "是的 - 恢复 %s", BaseName(backup_source));
        if (confirm_selection(confirm_install, tmp)) {
...
void custom_restore_menu(const char* backup_volume) {
    ...
    for (;;)
    {
        ...
        list[LIST_ITEM_VALIDATE] = strdup(">> 开始自定义恢复任务");

        if (reboot_after_nandroid) ui_format_gui_menu(menu_item_tmp, ">> 重新启动后完成", "(x)");
        else ui_format_gui_menu(menu_item_tmp, ">> 重新启动后完成", "( )");
        list[LIST_ITEM_REBOOT] = strdup(menu_item_tmp);

        if (volume_for_path(BOOT_PARTITION_MOUNT_POINT) != NULL) {
            if (backup_boot) ui_format_gui_menu(menu_item_tmp, "恢复 boot", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "恢复 boot", "( )");
        ...
        if (volume_for_path("/recovery") != NULL) {
            if (backup_recovery) ui_format_gui_menu(menu_item_tmp, "恢复 recovery", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "恢复 recovery", "( )");
            list[LIST_ITEM_RECOVERY] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_RECOVERY] = NULL;
        }

        if (backup_system) ui_format_gui_menu(menu_item_tmp, "恢复 system", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "恢复 system", "( )");
        list[LIST_ITEM_SYSTEM] = strdup(menu_item_tmp);

        if (volume_for_path("/preload") != NULL) {
            if (backup_preload) ui_format_gui_menu(menu_item_tmp, "恢复 preload", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "恢复 preload", "( )");
            list[LIST_ITEM_PRELOAD] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_PRELOAD] = NULL;
        }

        if (backup_data) ui_format_gui_menu(menu_item_tmp, "恢复 data", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "恢复 data", "( )");
        list[LIST_ITEM_DATA] = strdup(menu_item_tmp);

        set_android_secure_path(tmp);
        if (backup_data && android_secure_ext)
            ui_format_gui_menu(menu_item_tmp, "恢复 and-sec", DirName(tmp));
        else ui_format_gui_menu(menu_item_tmp, "恢复 and-sec", "( )");
        list[LIST_ITEM_ANDSEC] = strdup(menu_item_tmp);

        if (backup_cache) ui_format_gui_menu(menu_item_tmp, "恢复 cache", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "恢复 cache", "( )");
        list[LIST_ITEM_CACHE] = strdup(menu_item_tmp);

        if (backup_sdext) ui_format_gui_menu(menu_item_tmp, "恢复 sd-ext", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "恢复 sd-ext", "( )");
        list[LIST_ITEM_SDEXT] = strdup(menu_item_tmp);

        if (volume_for_path("/modem") != NULL) {
            if (backup_modem == RAW_IMG_FILE)
                ui_format_gui_menu(menu_item_tmp, "恢复 modem [.img]", "(x)");
            else if (backup_modem == RAW_BIN_FILE)
                ui_format_gui_menu(menu_item_tmp, "恢复 modem [.bin]", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "恢复 modem", "( )");
        ...
        if (volume_for_path("/radio") != NULL) {
            if (backup_radio == RAW_IMG_FILE)
                ui_format_gui_menu(menu_item_tmp, "恢复 radio [.img]", "(x)");
            else if (backup_radio == RAW_BIN_FILE)
                ui_format_gui_menu(menu_item_tmp, "恢复 radio [.bin]", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "恢复 radio", "( )");
        ...
        if (volume_for_path("/efs") != NULL) {
            if (backup_efs == RESTORE_EFS_IMG)
                ui_format_gui_menu(menu_item_tmp, "恢复 efs [.img]", "(x)");
            else if (backup_efs == RESTORE_EFS_TAR)
                ui_format_gui_menu(menu_item_tmp, "恢复 efs [.tar]", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "恢复 efs", "( )");
        ...
        if (volume_for_path("/misc") != NULL) {
            if (backup_misc) ui_format_gui_menu(menu_item_tmp, "恢复 misc", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "恢复 misc", "( )");
        ...
        if (volume_for_path("/wimax") != NULL && !twrp_backup_mode.value) {
            if (backup_wimax)
                ui_format_gui_menu(menu_item_tmp, "恢复 WiMax", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "恢复 WiMax", "( )");
        ...
        switch (chosen_item)
        {
            ...
            case LIST_ITEM_ANDSEC:
                ...
                if (!ignore_android_secure && get_num_extra_volumes() != 0)
                    ui_print("要强制恢复到第2存储，只保留一个 .android_secure 文件夹\n");
                break;
...
void custom_backup_menu(const char* backup_volume)
{
    ...
    for (;;)
    {
        ...
        list[LIST_ITEM_VALIDATE] = strdup(">> 开始自定义备份任务");
        ...
        if (backup_sdext) ui_format_gui_menu(menu_item_tmp, "备份 sd-ext", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "备份 sd-ext", "( )");
        list[LIST_ITEM_SDEXT] = strdup(menu_item_tmp);
        ...
        switch (chosen_item)
        {
            ...
            case LIST_ITEM_ANDSEC:
                ...
                ignore_android_secure ^= 1;
                if (!ignore_android_secure && get_num_extra_volumes() != 0)
                    ui_print("要强制从第二个存储备份，只保留一个 .android_secure 文件夹\n");
                break;
            case LIST_ITEM_CACHE:
                backup_cache ^= 1;
                break;
...
static void load_theme_settings() {
...
#else
    const char* headers[] = { "选择一个主题加载", "", NULL };
    ...
    if (confirm_selection("覆盖默认设置 ?", "是的 - 应用新主题") && copy_a_file(theme_file, PHILZ_SETTINGS_FILE) == 0) {
...
static void import_export_settings() {
    const char* headers[] = { "保存/恢复 设置", "", NULL };
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 4: {
                ...
                if (theme_file == NULL)
                    break;
                if (confirm_selection("删除选中的主题 ?", "是的 - 删除"))
...
{
    const char* headers[] = { "Recovery 设置", NULL };
    ...
    for (;;) {
        if (check_root_and_recovery.value)
            ui_format_gui_menu(item_check_root_and_recovery, "验证 Root 后退出", "(x)");
        else ui_format_gui_menu(item_check_root_and_recovery, "验证 Root 后退出", "( )");

        if (auto_restore_settings.value)
            ui_format_gui_menu(item_auto_restore, "自动恢复设置", "(x)");
        else ui_format_gui_menu(item_auto_restore, "自动恢复设置", "( )");
        ...
        switch (chosen_item) {
            ...
            case 3: {
                if (confirm_selection("重设所有Recovery设置吗?", "是的 - 重设到默认值")) {
                    char settings_copy[PATH_MAX];
                    sprintf(settings_copy, "%s/%s", get_primary_storage_path(), PHILZ_SETTINGS_FILE2);
                    delete_a_file(PHILZ_SETTINGS_FILE);
                    delete_a_file(settings_copy);
                    refresh_recovery_settings(0);
                    ui_print("所有设置已重置到默认值!\n");
                }
                break;
            }
#ifdef PHILZ_TOUCH_RECOVERY
            ...
            case 6: {
                ...
                ui_print("版本: " EXPAND(PHILZ_BUILD) " - " EXPAND(TARGET_COMMON_NAME) "\n");
                ui_print("CWM 版本: " EXPAND(CWM_BASE_VERSION) "\n");
                ui_print("狐狸也糊涂 汉化\n");
#ifdef PHILZ_TOUCH_RECOVERY
...
int show_install_update_menu() {
    ...
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes]     = "从自由浏览模式选择ZIP";
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 1] = "从上次安装目录选择ZIP";
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 2] = "从adb sideload安装ZIP";
    ...
    for (;;) {
        ...
        if (install_zip_verify_md5.value)
            ui_format_gui_menu(item_install_zip_verify_md5, "验证ZIP包md5数值", "(x)");
        else ui_format_gui_menu(item_install_zip_verify_md5, "验证ZIP包md5数值", "( )");
...
void show_choose_zip_menu(const char *mount_point) {
    ...
    const char* headers[] = { "选择一个ZIP应用", NULL };
...
void show_nandroid_restore_menu(const char* path) {
    ...
    const char* headers[] = { "选择镜像文件恢复", NULL };
...
void show_nandroid_delete_menu(const char* volume_path) {
    ...
    const char* headers[] = { "选择备份文件删除", NULL };
    char path[PATH_MAX];
    char tmp[PATH_MAX];
    char* file;
...
static void show_format_ext4_or_f2fs_menu(const char* volume) {
    ...
    if (is_data_media() && strcmp(v->mount_point, "/data") == 0) {
        ...
        ret = confirm_with_headers(confirm_headers, "是的 - 格式化该设备");
    } else {
        ret = confirm_selection(confirm, "是的 - 格式化该设备");
    }
...
void show_partition_format_menu() {
    ...
    for (;;) {
        ...
        if (!is_data_media()) {
...
#ifdef USE_F2FS
            list[formatable_volumes] = "切换 f2fs <-> ext4 迁移";
            ...
            list[formatable_volumes + 1] = NULL;
#ifdef USE_F2FS
            list[formatable_volumes + 1] = "切换 f2fs <-> ext4 迁移";
        ...
        if (is_data_media() && chosen_item == formatable_volumes) {
            ...
            ui_print("ext4 <-> f2fs 转换 %s\n", enable_f2fs_ext4_conversion ? "允许" : "禁止");
        }
#endif
    }
}
...
int show_partition_mounts_menu() {
    ...
    for (;;) {
        ...
        if (is_vold_ums_capable) {
            list[mountable_volumes] = "挂载USB大容量存储";
...
void choose_default_backup_format() {
    ...
    switch (chosen_item) {
        case 0: {
            ...
            ui_print("默认的备份格式设置为tar.\n");
            break;
        }
        case 1: {
            write_string_to_file(path, "dup");
            ui_print("默认的备份格式设置为dedupe.\n");
            break;
        }
        case 2: {
            write_string_to_file(path, "tgz");
            ui_print("默认的备份格式设置为tar + gzip.\n");
            break;
        }
    }
}
...
int show_nandroid_menu() {
    ...
    list[offset]     = "复制ROM为 update.zip";
    list[offset + 1] = "释放未使用的备份数据";
    list[offset + 2] = "其他nandroid设置";
...
void show_format_sdcard_menu(const char* path) {
    ...
    const char* headers[] = { "格式设备:", path, "", NULL };
    ...
    if (chosen_item < 0) // REFRESH or GO_BACK
        return;
    if (!confirm_selection("确认要格式化吗?", "是的 - 格式化该设备"))
        return;
...
void show_advanced_power_menu() {
    const char* headers[] = { "高级电源选项", "", NULL };
    ...
    for (;;) {
        if (is_data_media()) {
            if (use_migrated_storage())
                list[6] = "SD卡目录: /data/media/0";
            else list[6] = "SD卡目录: /data/media";
        }
        ...
        switch (chosen_item) {
            ...
            case 2: {
                if (confirm_selection("删除现有su ?", "是的 - 应用SuperSU")) {
...
int verify_root_and_recovery() {
    ...
    if (0 != lstat("/system/etc/.installed_su_daemon", &st)) {
        ...
        if (0 == lstat("/system/etc/install-recovery.sh", &st)) {
            if (st.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) {
                ui_SetShowText(true);
                if (confirm_selection("重启后将会恢复原来recovery. 修复?", "是的 - 禁止recovery恢复")) {
    ...
    if (0 == lstat("/system/bin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            ...
            if (needs_suid && (st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_SetShowText(true);
                if (confirm_selection("root访问权限可能丢失. 修复?", "是的 - 修复root (/system/bin/su)")) {
    ...
    if (0 == lstat("/system/xbin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            ...
            if (needs_suid && (st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_SetShowText(true);
                if (confirm_selection("root访问权限可能丢失. 修复?", "是的 - 修复root (/system/xbin/su)")) {
    ...
    if (!exists || su_nums != 1) {
        ui_SetShowText(true);
        if (confirm_selection("root访问权限缺失或被破坏. Root设备?", "是的 - 应用root (/system/xbin/su)")) {
