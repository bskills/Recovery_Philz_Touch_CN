...
void selective_load_theme_settings() {
    ...
    for(;;) {
        ...
        switch (chosen_item) {
            case 0: {
                if (confirm_selection("覆盖所有设置 ?", "是的 - 应用新主题") && copy_a_file(theme_file, PHILZ_SETTINGS_FILE) == 0) {
...
static void load_qcom_time_daemon(int on_start) {
    ...
    if (!file_found("/system/bin/time_daemon") ||
        ...
        if (on_start)
            draw_visible_text_line(5, "正在启动时间校正进程...", 1);
        else
            ui_print("正在启动时间校正进程...\n");
...
static void choose_background_image(const char* sd_path) {
    ...
    const char* headers[] = {"选择一张背景图.", NULL};
...
static void browse_background_image() {
    ...
    const char* headers[] = {"选择图片或颜色", "", NULL};
    int list_top_items = 5;
    char list_prefix[] = "图片从 ";
...
static void gestures_action_setup() {
    const char* headers[] = { "手势动作设置", NULL };
    ...
    for (;;) {
        ...
        while (gesture_action[i] != NULL) {
            ...
            ui_format_gui_menu(item_press_lift, "长按左边空白处", gesture_action[press_lift_action.value]);
            ui_format_gui_menu(item_press_move, "长按右边空白处", gesture_action[press_move_action.value]);
            i++;
        }
...
static void change_date_time_menu() {
    ...
    for (;;) {
        ...
        if (force_system_date) ui_format_gui_menu(item_force_system_date, "尝试强制在重启后保留", "(x)");
        else ui_format_gui_menu(item_force_system_date, "尝试强制在重启后保留", "( )");

        // qcom devices: try to directly parse the /data/time contents
        if (use_qcom_time_data_files.value) ui_format_gui_menu(item_parse_time_daemon_data_files, "解析时间校正数据", "(x)");
        else ui_format_gui_menu(item_parse_time_daemon_data_files, "解析时间校正数据", "( )");

        // menu to toggle load of time_daemon
        if (use_qcom_time_daemon.value)
            ui_format_gui_menu(item_qcom_time_daemon, "高通时间校正进程", "(x)");
        else ui_format_gui_menu(item_qcom_time_daemon, "高通时间校正进程", "( )");

        // menu to toggle use of time offset
        if (use_qcom_time_offset.value != 0)
            ui_format_gui_menu(item_qcom_time_offset, "使用 RTC 时间偏移...", "(x)");
        else ui_format_gui_menu(item_qcom_time_offset, "使用 RTC 时间偏移...", "( )");
        ...
        if (previous > 5)
            ...
            previous = 5;
        sprintf(item_next, "下一个 > (%s)", date_elements[next]);
        sprintf(item_previous, "上一个 < (%s)", date_elements[previous]);
        ...
        if (chosen_item == CHANGE_TIME_MENU_VALIDATE) {
            ...
            char* qcom_list[] = { "是的 - 应用时间校正数据", NULL };
            struct timeval tv;
            ...
            const char* qcom_headers[] = { "加载时间校正进程:", "只能为高通机子使用", "如果所有的手动模式失败", NULL };
            char* qcom_list[] = { "是的 - 加载时间校正进程", NULL };
            ...
            const char* qcom_headers[] = { "启用时间偏移:", "只能为高通机子使用", "如果所有的手动模式失败", NULL };
            char* qcom_list[] = { "是的 - 启用时间偏移", NULL };
...
static void show_time_settings_menu() {
    ...
    for (;;) {
        ...
        if (use_dst_time.value)
            ui_format_gui_menu(item_dst, "日光节约时间", "(x)");
        else ui_format_gui_menu(item_dst, "日光节约时间", "( )");
...
void show_touch_gui_menu() {
    const char* headers[] = { "图形界面设置", NULL };
    ...
    for (;;) {
        if (touch_to_validate.value == NO_TOUCH_SUPPORT)
            ui_format_gui_menu(item_touch, "触摸图形界面", "禁用");
        else if (touch_to_validate.value == TOUCH_HIGHLIGHT_ONLY)
            ui_format_gui_menu(item_touch, "触摸图形界面", "仅选择");
        else if (touch_to_validate.value == DOUBLE_TAP_VALIDATION)
            ui_format_gui_menu(item_touch, "触摸图形界面", "双击");
        else
            ui_format_gui_menu(item_touch, "触摸图形界面", "全部");

        if (menu_height_increase.value == MENU_HEIGHT_INCREASE_0)
            sprintf(tmp, "%ld (默认值)", MENU_HEIGHT_TOTAL);
        ...
        if (scroll_sensitivity.value == SCROLL_SENSITIVITY_0)
            sprintf(tmp, "%ld (默认值)", scroll_sensitivity.value);
        ...
        if (touch_accuracy.value == TOUCH_ACCURACY_0)
            sprintf(tmp, "%ld (默认值)", touch_accuracy.value);
        ...
        if (min_log_rows.value == 3) //defined by MIN_LOG_ROWS in ui.c
            sprintf(tmp, "%ld (默认值)", min_log_rows.value);
        ...
        if (set_brightness.value == BRIGHTNESS_DEFAULT_VALUE)
            sprintf(tmp, "%ld (默认值)", set_brightness.value);
        ...
        ui_format_gui_menu(item_dim_time, "屏幕昏暗延时", tmp);

        if (blank_timeout.value == 0)
            sprintf(tmp, "0 (禁用)");
        else sprintf(tmp, "%ld mn", blank_timeout.value / 60);
        ui_format_gui_menu(item_blank_time, "屏幕关闭延时", tmp);
...
static int make_update_zip(const char* source_path, const char* target_volume) {
    ...
    if (ret != 0) {
        return print_and_error("创建一个zip镜像时出错!\n", ret);
    }
...
static void custom_rom_target_volume(const char* source_path) {
    ...
    const char* headers[] = {"选择自定义ROM包", "", NULL};
    char* list[MAX_NUM_MANAGED_VOLUMES + 1];
    char list_prefix[] = "创建ROM在 ";
    char buf[80];
    memset(list, 0, sizeof(list));
    sprintf(buf, "%s%s", list_prefix, primary_path);
    list[0] = strdup(buf);
...
static void choose_nandroid_menu() {
    ...
    if (!file_found(tmp)) {
        ...
        sprintf(tmp, "是的 - 创建使用 %s", BaseName(file));
        if (confirm_selection("确认创建ROM?", tmp))
...
static void recovery_change_passkey() {
    ...
    for (i = 0; i < RECOVERY_LOCK_MAX_CHARS; ++i) {
        ...
        draw_visible_text_line(2, "* 新密匙 (6个字符) *", 1);
    ...
    for (i = 0; i < RECOVERY_LOCK_MAX_CHARS; ++i) {
        ui_update_screen();
        draw_visible_text_line(2, "* 重新输入密匙 *", 1);
        draw_visible_text_line(4, pass_display, 1);
        ...
        if (input_key != pass_key[i]) {
            ...
            draw_visible_text_line(2, "* 密匙不匹配! *", 1);
...
void check_recovery_lock() {
    ...
    if (key_err >= RECOVERY_LOCK_MAX_ERROR) {
        ...
        draw_visible_text_line(2, "密匙错误!", 1);
...
void show_recovery_lock_menu() {
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 1: {
                if (confirm_selection("禁用Recovery锁 ?", "是的 - 禁用")) {
...
void print_libtouch_version(int onscreen) {
    if (onscreen)
        ui_print("图形界面版本: " EXPAND(LIBTOUCH_GUI_VERSION) "\n");
    else
