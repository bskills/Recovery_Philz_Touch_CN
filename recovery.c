...
main(int argc, char **argv) {
    ...
    if (update_package != NULL) {
        ...
        if (status != INSTALL_SUCCESS) ui_print("数据擦除失败.\n");
    } else if (wipe_cache) {
        if (wipe_cache && erase_volume("/cache")) status = INSTALL_ERROR;
        if (status != INSTALL_SUCCESS) ui_print("缓存擦除失败.\n");
    } else if (wipe_media) {
        if (is_data_media() && erase_volume("/data/media")) status = INSTALL_ERROR;
        if (status != INSTALL_SUCCESS) ui_print("媒体文件擦除失败.\n");
    } else if (sideload) {
